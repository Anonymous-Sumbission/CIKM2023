...
test_queries=pd.read_csv("MusQuE/data/sample_test_queries.tsv", sep="\t", header=None, names=["query"])
all_queries=list(test_queries["query"].unique())
...
class MusQuE(torch.nn.Module):
    ...
    def combination(self,terms, num):
        x=itertools.combinations(terms, num)
        final=[list(f) for f in x]
        return(final))


    def multi_stage_colbert(self, query, data, stage, all_terms, bert_results, previous_best_score, previous_best_query):
        ...
        if (query_scores.shape[0]==0):
            ...
            if (current_best_score<previous_best_score):
                ...
                return(terms, current_best_score, stage_flag, best_query)
            else:
                stage_flag=0
    ...
    def final_stage(self,stage_flag, content_dict, query, data, terms, best_query):
        ...
        all_scores, tmp_scores_df, all_pos_neg_results, all_pos_neg_labels=self.catbert(query,new_query,content_dict)
        labels=torch.ones((pos_scores.size()))
        labels.requires_grad=True
        labels=labels.to(device)
        return(all_scores, tmp_scores_df)
    ...
    def catbert(self,query,new_query, contents):
        ...
        tmp_scores_df["score"]=all_scores_list
        tmp_scores_df["doc_id"]=all_ids
        tmp_scores_df["query"]=query
        tmp_scores_df["label"]=all_pos_neg_labels.detach().numpy()
        all_pos_neg_labels=all_pos_neg_labels.to(device)
        return(all_scores, tmp_scores_df, all_scores, all_pos_neg_labels)

t=MusQuE()
t.load_state_dict(torch.load("/MusQuE/Data/trained_model"))
device="cuda" if cuda.is_available() else "cpu"
t=t.to(device)
t.eval()

def multi_stage_ranking(all_queries, bert_results,terms_df, MusQuE_scores_df):
    t.eval()
    ...
    for q in range(len(all_queries)):
        try: 
            ...
            terms=list(terms_df1["terms"].unique())
            for term1 in terms:
                    for term2 in terms:
            ...
            content_dict=t.tokenized_content(self, query)
            data_tmp_=t.bert_embedding(content_dict, all_queries[q])
            bert_results_=bert_results.loc[bert_results["query"]==all_queries[q],]
            if (len(terms)<2):    
                tmp_scores_df, new_query=t.final_stage("1", content_dict,all_queries[q], data_tmp, terms, "")
                MusQuE_scores_df=MusQuE_scores_df.append(tmp_scores_df)
            else:
                for stage in range(1,t.number_of_stages):
                    if (stage==1):
                        terms,current_best_score, stage_flag, best_query=t.multi_stage_colbert(all_queries[q], data_tmp, stage, terms, bert_results_, 10000000000000, all_queries[q])
                        if (stage_flag==1):
                            print("stage flag is zero")
                        else:
                            break
                    else:
                        terms, current_best_score, stage_flag, best_query=t.multi_stage_colbert(all_queries[q], data_tmp, stage, terms, bert_results_, current_best_score, best_query)
                        if (stage_flag==0):
                            break
                            self,stage_flag, content_dict, query, data, terms, best_query
                if (stage_flag==1):
                    tmp_scores_df, new_query=t.final_stage(stage_flag, content_dict,all_queries[q], data_tmp, terms, best_query)
                    MusQuE_scores_df=MusQuE_scores_df.append(tmp_scores_df)
                else:
                    tmp_scores_df, new_query=t.final_stage(stage_flag, content_dict,all_queries[q], data_tmp, terms, best_query)
                MusQuE_scores_df=MusQuE_scores_df.append(tmp_scores_df)
    ...
    MusQuE_scores_df.to_csv("/MusQuE/MusQuE_scores.tsv", sep="\t", index=False)
